...
network_translate_hostname(char* hostname, network_address_t address) {
  ...
  if(isalpha(hostname[0])) {
    ...
    if (host == NULL)
      ...
      //printf("address[0] = %x",(int)address[0]);
      //printf("address[1] = %x",(int)address[1]);
      return 0;
    }
  }
  else {
...
int network_poll(void* arg) {
  ...
  sigset_t set;
  sigemptyset(&set);
  sigaddset(&set,NETWORK_SIGNAL);
  sigaddset(&set,CLOCK_SIGNAL);
...
void start_network_poll(interrupt_handler_t network_handler, int* s) {
  ...
  if (sigaction(NETWORK_SIGNAL, &sa, NULL) == -1)
    AbortOnError(0);
